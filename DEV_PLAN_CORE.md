...
> 配套文档：`GDD.md`（权威设计）、`PROJECT_SETUP.md`、`EXTENSION_GUIDE.md`
> 你的目标：做出一个**大体可玩、包含所有核心机制**的版本，然后交给做 gimmick 的人和做地图的人继续。
> 团队分工：你=游戏核心 / B=gimmick（机关与扩展概念）/ C=地图制作。
> 前提：选做部分（hud 界面坍塌、pause、fourthwall）一律**视为要做**。
...
### 0.1 你现在手里已有的东西
上一轮已产出这些脚本（在 `scripts/`）：`sacrifice_manager.gd`、`player.gd`、`player_config.gd`、`blue_object.gd`、`altar.gd`、`sacrifice_input.gd`、`hud.gd`（文字版）、`demo_level.gd`（一次性测试台）。**本计划是在这套地基上往上盖，不是重写。**
...
**交付物**：
- 按 `PROJECT_SETUP.md` 建好项目：autoload `Sacrifice`、Input Map、`Main.tscn` 主场景、`scripts/`+`scenes/`+`tuning/` 目录结构。
...
**交付物**：
- 概念初始全部锁定，只有走到祭坛才解锁（`SacrificeInput` 只允许切换已解锁的）。
- `Altar.tscn` 支持三种 Action（UNLOCK / SET_SLOTS / PERMANENT_SACRIFICE），Inspector 可配。
- 双槽神龛：叠放两个祭坛（SET_SLOTS 2 + PERMANENT_SACRIFICE jump），一次触发。
- 永久牺牲 jump 后跳跃键失效。
- **世界内控制提示**：走到祭坛时在其旁边显示该做什么（如"按 1 切换"的牌子/浮字），不做独立 UI 面板。

**建议提示词**：
> 实现解锁流程：所有概念初始锁定，SacrificeInput 只允许切换已解锁的概念。让 Altar 预制体在 Inspector 里可选 UNLOCK/SET_SLOTS/PERMANENT_SACRIFICE 三种行为。做双槽神龛——叠两个 Altar，一个把槽位设为 2、一个永久牺牲 jump，玩家进入同时触发。永久牺牲 jump 后跳跃键无效。再加世界内提示：玩家靠近祭坛时在旁边显示一行该操作的文字。

**完成判定（玩一遍）**：
- [ ] 游戏一开始按 1/2 无反应（未解锁）。
- [ ] 走到重力祭坛后按 1 才生效；蓝祭坛后按 2 才生效。
- [ ] 走到双槽神龛：HUD 槽位变 2，同时跳跃键此后失效。
...
## 步骤四：全部 UI 牺牲（hud 界面坍塌 + pause + fourthwall 结局）（一次大会话，较重）

**目标**：把三个 UI 类牺牲全部做出来（选做即必做）。这一步最杂，安排大块时间。

**交付物**：
- **HUD 升级**：从文字换成图标版，每个概念一枚图标，三态（灰/常态/高亮），并把 HUD 本身接进信号总线（它也是一个"会对牺牲反应的对象"）。
- **pause**：实现真正的暂停功能 + `pause` 牺牲（永久牺牲后暂停键失效）。作为双槽的**备选价格**入口（与 jump 二选一，见 GDD §2.6，别对同一升级收两份价）。
...
**建议提示词**：
> 分四块做 UI 牺牲。①把 HUD 改成图标版，每概念一图标三态，并让 HUD 监听 Sacrifice 信号。②实现暂停功能和 pause 牺牲：永久牺牲 pause 后暂停键失效。③实现 hud 牺牲：永久牺牲后状态 UI 解体消失；牺牲瞬间在预设的 Marker2D 落点生成一次性实体平台，做成可复用场景（落点在编辑器里加/挪 Marker2D 调整，不写死坐标）。④fourthwall 结局：最终祭坛触发后让 HUD 逐个掉落、场景元素淡出、显示一行结束文字然后黑屏，全程纯视觉不操作操作系统窗口。

**完成判定（玩一遍）**：
- [ ] HUD 图标随解锁/激活/槽位变化正确三态显示。
- [ ] 能暂停；牺牲 pause 后暂停键无效。
- [ ] 牺牲 hud 后状态 UI 消失；预设落点坠落生成实体平台，之前够不到的高台/缺口现在能踩上去了。
- [ ] 触发最终祭坛：界面和场景依次解体到黑屏，且窗口本身没被乱动。
...
## 步骤五：房间系统 + 打通一条粗略但完整的整合关（一次大会话）

**目标**：给 C 一套能摆地图的房间框架，并自己拼出一条从头到尾覆盖所有机制的**粗略**关卡，证明所有机制能串起来通关。

**交付物**：
- 房间/关卡框架：房间场景模板 + 房间间切换（门/边界触发）+ 相机处理。做成 C 能复制的模板。
- 一条粗略整合关，按 GDD §5.2 的 R1→R6 流程串起：教重力→测重力→教蓝并看见门槛→双槽神龛→双牺牲打通蓝竖井→fourthwall 结局。**几何用占位方块即可，不求好看**——精修交给 C。
- 严格自查 GDD §5.4 硬约束：牺牲 jump 之后到通关全程不需要跳（逐房间实走确认）。

**建议提示词**：
> 做一个房间系统：一个房间场景模板，用触发区做房间间切换，相机跟随。做成 C 能直接复制来建新房间的模板。然后用占位方块拼一条完整关卡，按这个顺序串起来：教重力、重力竖井、教蓝且能望见蓝竖井入口、双槽神龛（牺牲 jump）、需要同时开重力和蓝的蓝刺竖井、fourthwall 结局。几何粗糙没关系。特别确认：牺牲 jump 之后的所有房间都不需要跳就能过。
...
**解锁**：C 拿到房间模板和一条参考关，可以正式做地图；此时"大体可玩含所有核心机制"的版本已达成。

---

## 步骤六：音频钩子 + 打磨 + 交接打包（一次会话）

**目标**：补上音频接口和整体反馈，整理成能"扔出去"的交接包。

**交付物**：
- 简单 AudioManager：牺牲切换音、落地音、祭坛音、1 首 BGM；预留 `sound` 牺牲要用的"全局静音"钩子（供 B 之后接）。
...
- [ ] 一段话说明"如何加一个新概念"（照 `EXTENSION_GUIDE.md` §D）：复制模板→改 id→放个 UNLOCK 祭坛→在 SacrificeInput 加键。
...
- [ ] 一条可参考的整合关。
- [ ] 两条必须遵守的硬约束：GDD §5.4（牺牲 jump 后不需跳）和 §8.7（别设计"在蓝墙内恢复蓝"的谜题）。

**通用**：
- [ ] 所有可配置项都在 Inspector / .tres 里，B 和 C 基本不用改代码。
- [ ] `AI_CONTEXT.md`（0.2 那段）留给他们，让他们的 vibe coding 也守同样的架构铁律。
...
5. **别让 AI 顺手扩范围**。它可能主动加战斗、加菜单——按 GDD §10.1，不在清单里的一律不做。
