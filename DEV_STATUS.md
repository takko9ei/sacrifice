...
**不应存在但需要警惕的脚本**：`observation_gate.gd`——已删除，如果又出现说明清理不完整（见第5节第5条）。
...
**不应存在但需要警惕的场景**：`TestRoom.tscn`（见第5节第4条）、`ObservationGate.tscn`（见第5节第5条）——两者按设计都应该已被删除，但写这份快照时核对发现它们又出现在磁盘上，本轮已按范围限制只记录不处理，下一次有代码改动权限的会话应确认并删除。
...
5. **（待办，本次快照核对时新发现）`ObservationGate.tscn` 又出现在磁盘上，需要删除**：旧的"观测坍缩机关"设计早就被"图标坠落变平台"取代，对应脚本 `scripts/observation_gate.gd` 确认已删除、项目里没有任何地方实例化或引用 `ObservationGate.tscn`。但写这份快照时核对 `scenes/` 目录，发现 `scenes/ObservationGate.tscn` 这个场景文件本身又出现了——它引用的脚本 `res://scripts/observation_gate.gd` 并不存在，是一个指向缺失脚本的孤立/损坏场景文件（不会影响运行，因为没有任何场景实例化它，但在编辑器里打开会报"脚本缺失"）。这是和第4条 `TestRoom.tscn` 同类型的"已删除文件重新出现"现象，原因同样未知。**本次按用户要求只改动 `DEV_STATUS.md`，没有顺手删除它**——下一次有代码改动权限的会话应该确认并删除 `scenes/ObservationGate.tscn`。
...
**优先待办（比步骤六本身更紧急）**：清理第5节第4、5条提到的两个"复活"的残留文件——`scenes/TestRoom.tscn`、`scenes/ObservationGate.tscn`。确认删除前先 grep 一下项目确保没有任何场景/脚本引用它们（本文档写就时确认过都是孤立文件），删除后更新本文件第5节对应条目。
