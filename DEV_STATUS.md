...
## 2. 已完成的功能（对应 DEV_PLAN 步骤一 → 五，含步骤五后的一次追加改动）
...
- 切换反馈：`HUD.tscn` 的 `FlashOverlay`（白色半透明 ColorRect）每次切换闪一下（`hud.gd::_play_flash()`）。项目已决定不做音频，这个反馈纯视觉，没有配套音效（见第2节"步骤五后"）。
...
**当前项目里只有两个 UI 牺牲：`hud` 和 `fourthwall`。`pause` 不是第三个 UI 牺牲，它已经被完全移除，不存在于当前实现中。**

- **`pause` 现状（重要，容易被旧记忆或旧文档误导）**：`pause` 曾经是第三个 UI 牺牲（独立测试祭坛，牺牲后让 Escape 失效，但从未真正接入双槽升级），已被**整体删除**——`pause_controller.gd`/`PauseController.tscn` 两个文件已删除；当时挂着 `AltarPause`/`PauseController` 节点的主场景（`TestRoom.tscn`）后来也因为步骤五的场景整合而整体删除了（见下文"步骤五"）；`GDD.md`/`DEV_PLAN_CORE.md` 里所有相关描述已同步删掉。现在项目里**没有任何暂停功能**，Escape 键不绑定任何游戏逻辑。唯一残留：`project.godot` 的 Input Map 里 `pause`（Escape）这个动作定义还留着但无人监听，因为改 Project Settings 不在 AI 权限内（见第5节第6条）。
...
- **`hud`（界面坍塌）现状——⚠️ 设计中途改过，当前实现是"图标坠落变平台"，不是最初的"观测坍缩机关"**：
  - **旧设计（已完全退役，脚本已删除，但对应场景文件在磁盘上出现过重复复活，见第5节第5条）**：一种叫"观测坍缩机关"的门——牺牲 `hud` 前是实体挡路，牺牲后变可穿过。对应脚本 `scripts/observation_gate.gd` 已被删除，游戏逻辑上**不存在**任何地方引用或实例化 `ObservationGate.tscn`。
  - **新设计（当前实际生效的实现）**：`hud_collapse_platforms.gd`（挂在 `HudCollapsePlatforms.tscn`，纯 `Node2D`）监听 `Sacrifice.concept_permanently_sacrificed`，收到 `id == "hud"` 时，遍历自己所有 `Marker2D` 子节点，在每个 Marker2D 的位置用代码现造一个 `StaticBody2D`（碰撞层 1，矩形碰撞体+同尺寸 `Polygon2D` 视觉），从 `目标位置 + Vector2(0, -drop_height)` 用 Tween（`TRANS_QUAD`/`EASE_IN`）坠落到目标位置——效果是"HUD 图标砸进关卡变成几块可踩的实体平台"。落点完全由在编辑器里加/挪 `Marker2D` 子节点配置，脚本里没有任何写死坐标。
...
- **`fourthwall`（结局）现状**：`ending_sequence.gd`（挂在 `EndingSequence.tscn`，`layer = 10`，`process_mode = Always`）监听 `concept_permanently_sacrificed`，`id == "fourthwall"` 时：`get_tree().paused = true` → 用一条 Tween 序列依次淡出 HUD（`hud_fade_target_path` 在场景里手动接到 `../HUD/Layout`）→ 黑色 `Overlay` 淡入到 `dissolve_alpha`（0.85，未全黑）→ 文字 `Label`（默认"Thank you for playing."）淡入 → 停留 → 文字淡出的同时 `Overlay` 继续淡到全黑（并行）。全程纯视觉，不碰 OS 窗口。触发后游戏树整体暂停，但 `RestartController`（`process_mode = Always`）仍能响应 R 键重开。

### 步骤五 · 房间模板 + 单场景整合关 —— ✅ 完成（无房间切换系统，用户明确选择单场景方案）
- **场景取舍决定（已执行完毕）**：`IntegrationLevel.tscn` 定为**唯一正式关卡**，以后要加内容一律在它上面加长/加区域（见 `DEV_PLAN_CORE.md` 步骤五的架构决定）。人工已把 `project.godot` 的 `run/main_scene` 改指向 `IntegrationLevel.tscn`（`uid://bkdctvarex6yi`），AI 随后删除了 `scenes/TestRoom.tscn` 并清理了代码里对它的注释引用（`restart_controller.gd`）。**`TestRoom.tscn` 按设计已不应存在于项目中**——但见第5节第4条，这个文件曾经在删除之后又诡异地重新出现过一次，接手者需要留意。
- `RoomTemplate.tscn` + 纯注释脚本 `room_template.gd`：给 C 的可复制样板，一个 `Ground`（`Ground.tscn` 实例，层1）+ 一个 `ExampleAltar`（`Altar.tscn` 实例）+ 一个 `ExampleMechanism`（`BlueObject.tscn` 实例，纵向拉伸演示缩放用法）。脚本顶部注释写明"复制这个场景改坐标/改 concept_id 就是新房间，不要碰 sacrifice_manager.gd/altar.gd/blue_object.gd"。
...
| `sacrifice_input.gd` | `Node` | `@export var bindings: Dictionary` 把输入动作名映射到 concept_id，按键触发 `Sacrifice.toggle()`（仅对已解锁概念生效）。挂在 `SacrificeInput.tscn` 上。 |
...
| `restart_controller.gd` | `Node`，`process_mode = Always` | 按 `restart`（R）：`Sacrifice.reset()` + 取消暂停 + 重载当前场景。挂在 `RestartController.tscn` 上。 |
...
**不应存在但需要警惕的脚本**：`observation_gate.gd`——已删除，如果又出现说明清理不完整（历史上其配套场景 `ObservationGate.tscn` 曾复现过，见第5节第5条）。

### `scenes/`
| 场景 | 结构 | 备注 |
|---|---|---|
| `Player.tscn` | `CharacterBody2D`(player.gd) > `CollisionShape2D` + `AnimatedSprite2D`(占位帧) + `Camera2D` | `config` 指向 `tuning/default.tres`；`collision_layer = 2`。⚠️ `sprite_path` 在场景文件里被序列化成字面 `null`，见第5节已知问题第2条。 |
...
| `IntegrationLevel.tscn` | 步骤五的 R1→R6 整合关，**当前项目实际的主场景**（见第7节"运行方式"） | 详见第2节步骤五描述。步骤一~四阶段曾用 `TestRoom.tscn` 作为手感/机制测试房，按设计已删除退役，测试与实际关卡都并入这一个场景。所有地面/平台/天花板/墙已改为 `Ground.tscn` 实例，`SacrificeInput`/`RestartController` 节点也已改为对应预制体实例（见第2节"预制体化整理"）。 |

**不应存在但需要警惕的场景**：`TestRoom.tscn`（见第5节第4条）、`ObservationGate.tscn`（见第5节第5条）——两者都曾在磁盘上复现过，当前均已确认删除；如果接手时又发现其中之一复活，直接删除即可。
...
5. 碰撞层：世界实体（地形、`Ground`、`BlueObject`、hud 坍塌平台）= Layer 1；玩家 = Layer 2；`Altar` 是 `Area2D`，`collision_layer = 0` / `collision_mask = 2`（只检测玩家进入，不参与物理碰撞）。
...
10. 地面/平台/天花板/墙一律用 `Ground.tscn` 实例（`size`/`color` 导出），不要再手写 `StaticBody2D`+`CollisionShape2D`+`Polygon2D` 三件套——这是步骤五后新增的约定，`ground.gd` 是唯一负责同步碰撞形状与可见多边形的地方。
11. 项目不做音频。不要添加 `AudioStreamPlayer`、音效播放逻辑或任何"预留音频钩子"的占位代码——这是一个已经拍板的范围变更，不是临时遗漏。
12. AI 助手侧的铁律（来自 `CLAUDE.md`，重复强调）：禁止一切 git 操作；禁止修改 Project Settings（autoload/Input Map/主场景由人类手动配置）；只做当前 DEV_PLAN 步骤范围内的事。
...
4. **（已解决，但请留意第5条同类现象）主场景已切到 `IntegrationLevel.tscn`，`TestRoom.tscn` 应已退役删除**：人工已把 `project.godot` 的 `run/main_scene` 改为 `IntegrationLevel.tscn`（`uid://bkdctvarex6yi`），AI 随后删除了 `scenes/TestRoom.tscn` 并清理了代码注释里的引用（`restart_controller.gd`）。现在按 F5 应该直接跑完整的 R1→R6 整合关，含 hud 演示，不需要手动在编辑器打开单独运行。**但这个文件在删除之后，曾经在某次未知的操作后又重新出现在磁盘上过一次，当时已再次删除**——原因未查明，怀疑与本机 Godot 编辑器的某种缓存/自动写回行为有关，不是人工手动恢复的。如果接手时又发现 `scenes/TestRoom.tscn` 存在，直接删除即可（并确认 `project.godot` 主场景仍指向 `IntegrationLevel.tscn`），不代表本文档记录有误。
5. **（已解决）`ObservationGate.tscn` 曾在磁盘上复现，本次会话已删除**：旧的"观测坍缩机关"设计（牺牲 `hud` 前挡路、牺牲后可穿过的门）早就被"图标坠落变平台"（`hud_collapse_platforms.gd`）取代，对应脚本 `scripts/observation_gate.gd` 已确认删除。上一次快照核对时发现场景文件 `scenes/ObservationGate.tscn` 本身又出现在磁盘上——它引用的脚本 `res://scripts/observation_gate.gd` 并不存在，是一个指向缺失脚本的孤立/损坏场景文件。本次会话已重新 grep 全项目（`.gd`/`.tscn`/`.tres`/`.godot`）确认没有任何场景/脚本引用或实例化它之后，将其删除。这是和第4条 `TestRoom.tscn` 同类型的"已删除文件重新出现"现象，原因同样未知（疑似本机 Godot 编辑器缓存行为）；如果接手时又发现 `scenes/ObservationGate.tscn` 存在，直接删除即可。
6. **`pause` 牺牲玩法已整体删除，但 Input Map 里的 `pause`（Escape）动作定义还留着**：`pause_controller.gd`/`PauseController.tscn` 已删除，当时挂着 `AltarPause`/`PauseController` 节点的主场景（`TestRoom.tscn`）本身后来也整体删除了，`GDD.md`/`DEV_PLAN_CORE.md` 里所有 `pause` 相关描述已同步删掉。但 `project.godot` 的 Input Map 仍定义着 `pause`（Escape）这个动作——按 `CLAUDE.md` 铁律，Project Settings 只能由人类在编辑器里手动改，AI 不能删。**这是一个预期内的、无害的孤立绑定：现在没有任何脚本监听它，按 Escape 不会有任何反应。** 如果人类想彻底清掉，需要自己去编辑器的 Input Map 面板删除这一行。

---

## 6. 下一步要做什么

**当前进度：步骤五（含步骤五后的 hud 机制重做，以及步骤五后追加的预制体化+音频下线+文档一致性修正）已完成，尚未开始步骤六。**

**清理已完成**：第5节第4、5条提到的两个"复活"残留文件——`scenes/TestRoom.tscn`、`scenes/ObservationGate.tscn`——均已确认删除（`ObservationGate.tscn` 于本次会话删除，删除前已重新 grep 全项目确认无任何引用）。不再是优先待办，可以直接进入步骤六。
...
- 一段"如何加一个新概念"的说明：复制样板→改 `concept_id`→摆一个 `UNLOCK` 类型的 `Altar`→在 `SacrificeInput.bindings` 里加一行绑键（GDD §7.6"扩展模式"）。
- 明确储备概念（`friction`/`time`/`sound`）由 B 实现，可复用上面两个样板的模式。

**交给 C（地图/关卡设计）需要准备好**：
- `RoomTemplate.tscn` + `room_template.gd` 的用法说明，以及更完整的 `scenes/HOW_TO_BUILD_A_LEVEL.md` 搭建教程（已交付，见第3节场景清单）。
...
（Input Map 里还有一个 `pause`（Escape）动作定义，但没有任何脚本监听它，见第5节第6条。）

### 主场景
`project.godot` 的 `run/main_scene = "uid://bkdctvarex6yi"`，对应 **`scenes/IntegrationLevel.tscn`**。按 F5 实际跑的就是步骤五的完整 R1→R6 整合关，涵盖 gravity/blue/双槽神龛/永久牺牲jump/hud 图标坠落平台演示/fourthwall结局全部机制。`TestRoom.tscn`（步骤一~四阶段的手感/机制测试房）按设计已退役删除（但见第5节第4条的复活现象）。
